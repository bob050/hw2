...
#pragma warning(disable: 4127)
    ...
    do 
    {
		...
		{
			print( "SetEndOfFile( ) failed, gle=0x%08x",  GetLastError() );
			break;
		}
	...
	)
{
	...
	if (TRUE == IsReadOnly())
	{
		if (TRUE != ReadOnly) 
		{
			print("file handle is read-only!" );
	...
	if (NULL == mFileView)
	{
		print("MapViewOfFile(high=0x%08x, log=0x%08x, bytes to map=%u) failed, gle=0x%08x", 
	...
	)
{
	...
	if(TRUE != DT_SUCCEEDED(status))
	{
		print("FIOReference() failed. status=0x%08x", 
			status);
		return status;
	}
	...
	{
		print("exception. code=0x%08x", GetExceptionCode());
